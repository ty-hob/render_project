...
//does the parsing of object types sphere, triangle, plane and returns the requested pointer of an objects parameter.
...
// function for finding the closes object given all the objects in sceene and ray star pos and direction
// if the closest_object distance value is -1 the pointer to object value is garbage and should be ignored
closest_object get_closest_object(object_manager* objm, vect3d start_pos, vect3d ray_direction){
	...
	if(objm->sceene_object_count != 0){
		...
		cobj.distance = check_intersection(ray_direction, start_pos, objm->sceene_objects[0].object_pointer);
	...
	// if more than one objects exists find the closest one by checking them all and updating the closes obj value if a
	// closer object is found to the starting point
	for(int i=1; i<objm->sceene_object_count; i++){
		check_intersection = get_object_parameter('i', objm->sceene_objects[i]);
		distance_to_object = check_intersection(ray_direction, start_pos, objm->sceene_objects[i].object_pointer);
...
// given a point in sceena and a object that that point is on calculate the shadeing of that given point.
rgb_color get_color_of_point(vect3d ray_direction, vect3d intersection_point, closest_object cobj, object_manager* objm, camera* camera_object, rgb_color ambient_light, int reflection_count, int refraction_count){
	...
	// get suface normal
	vect3d (*get_normal)(vect3d, void*) = get_object_parameter('n', cobj.c_object);
	vect3d suface_normal = get_normal(intersection_point, cobj.c_object.object_pointer);
	...
	for(i=0; i<objm->light_object_count; i++){
		...
		to_light_direction = make_unit_vect(intersection_point, objm->light_objects[i].pos);
		...
		point_color.r += cos_theta * mat->diffuse_color.r * objm->light_objects[i].diffuse_light_color.r;
		point_color.g += cos_theta * mat->diffuse_color.g * objm->light_objects[i].diffuse_light_color.g;
		point_color.b += cos_theta * mat->diffuse_color.b * objm->light_objects[i].diffuse_light_color.b;
		...
		cos_alpha = pow(maxv(0, dot(make_unit_vect(intersection_point, camera_object->camera_pos), scale(reflect_ray(to_light_direction, suface_normal), -1))), mat->shininess_const);
		//      pow(dot(light_reflection, to_view), shinines_cont) * specular_light_color * specular_reflection_const
		point_color.r += cos_alpha * objm->light_objects[i].specular_light_color.r * 1;
		point_color.g += cos_alpha * objm->light_objects[i].specular_light_color.g * 1;
		point_color.b += cos_alpha * objm->light_objects[i].specular_light_color.b * 1;
	...
	if(reflection_count < MAX_REFLECTIONS){
		if(mat->reflectivenes_const > 0){
			...
			closest_object reflect_cobj = get_closest_object(objm, intersection_point, ray_direction);
			// check if an object was hit by traced ray
			if(reflect_cobj.distance != -1){
				// get the color of that point
				rgb_color reflection_color = get_color_of_point(ray_direction, add(intersection_point, scale(ray_direction, reflect_cobj.distance)), reflect_cobj, objm, camera_object, ambient_light, reflection_count+1, 0);
				// aplly the color
	...
	if(refraction_count < MAX_REFRACTIONS){
		if(mat->refractivenes_const > 0){
			...
			closest_object refract_cobj = get_closest_object(objm, add(intersection_point, scale(ray_direction, 0.1)), ray_direction);

			if(refract_cobj.distance != -1){
				rgb_color refraction_color = get_color_of_point(ray_direction, add(intersection_point, scale(ray_direction, refract_cobj.distance+.1)), refract_cobj, objm, camera_object, ambient_light, 0, refraction_count+1);
...
// returns how many times a point on an object is in shadow of a light by other objects (not it self)
int trace_shadows(object_manager* objm, closest_object cobj, vect3d intersection_point){
	int obscursions = 0;
	...
	for(int l=0; l<objm->light_object_count; l++){
		...
		ray_direction = make_unit_vect(intersection_point, objm->light_objects[l].pos);
		...
		for(int i=0; i<objm->sceene_object_count; i++){
			if(cobj.c_object.object_pointer != objm->sceene_objects[i].object_pointer){
				check_intersection = get_object_parameter('i', objm->sceene_objects[i]);
				distance_to_object = check_intersection(ray_direction, intersection_point, objm->sceene_objects[i].object_pointer);
		...
		if(distance_to_closest_objscursion < vector_length(sub(intersection_point, objm->light_objects[l].pos)) && distance_to_closest_objscursion > 0){
			obscursions += 1;
		}
...
vect3d refract_ray(closest_object cobj, vect3d ray_direction, vect3d* intersection_point){
	...
	if(cobj.c_object.type == 's'){
		ray_direction = add(scale(ray_direction, n), scale(suface_normal, (n*cosI) - cosT));
		float d = sphere_intersection(ray_direction, add(*intersection_point, scale(ray_direction, 0.1)), cobj.c_object.object_pointer);
		...
		if(d != -1){
			...
			*intersection_point = add(*intersection_point, scale(ray_direction, d));
			...
			suface_normal = get_normal(*intersection_point, cobj.c_object.object_pointer);
			...
			return add(scale(ray_direction, n), scale(suface_normal, (n*cosI) - cosT));
...
vect3d reflect_ray(vect3d ray_direction, vect3d suface_normal){
	return sub(ray_direction, scale(suface_normal, 2*dot(ray_direction, suface_normal)));
}

// return the smallest distance to intersecton point from starting point
float sphere_intersection(vect3d ray_direction, vect3d start_pos, void* obj_prt){
...
// returns the distance to a triangle object. if ray has not intersected a triangle then returns -1
float triangle_intersection(vect3d ray_direction, vect3d start_pos, void* obj_prt){
	triangle* self = (triangle*)obj_prt;
	// get normal
	vect3d normal = self->get_normal(ray_direction, self); // ray direction just forr filler value. is not used in calculation
		...
		if(d <= 0){
			...
			if(dot(normal, cross(sub(self->p2, self->p1), sub(p, self->p1))) > 0){
				if(dot(normal, cross(sub(self->p3, self->p2), sub(p, self->p2))) > 0){
					if(dot(normal, cross(sub(self->p1, self->p3), sub(p, self->p3))) > 0){
						return d;
					}
				}
			}		
...
// returns the distance to intersiction point to a plane. if ray and plane are paralel then returns -1
// if distance is negative returns -1
float plane_intersection(vect3d ray_direction, vect3d start_pos, void* obj_prt){
...
// claculates the surface normal (unit vect) for an given sphere object and point of intersection.
...
// returns the suface normal of an triangle object. function has parameeter intersecton for compatability with sphere object
vect3d triangle_normal(vect3d intersection_point, void* obj_prt){
	triangle* self = (triangle*)obj_prt;
	if(self->invert_normal){
		return scale(normalize(cross(sub(self->p2, self->p1), sub(self->p3, self->p1))), -1);
	}else{
		return normalize(cross(sub(self->p2, self->p1), sub(self->p3, self->p1)));
	}
}

// returns the surface normal of plane object. function has unused variables for compatability for object manager
...
void add_sphere(object_manager* objm, float cx, float cy, float cz, float radius, material* mat){
	...
	objm->sphere_objects[objm->sphere_object_count].check_intersection = &sphere_intersection;
	objm->sphere_objects[objm->sphere_object_count].get_normal = &sphere_normal;

	objm->sceene_objects[objm->sceene_object_count].type = 's';
	objm->sceene_objects[objm->sceene_object_count].object_pointer = &objm->sphere_objects[objm->sphere_object_count];

	objm->sphere_object_count += 1;
	objm->sceene_object_count += 1;
}

// adds triangle object to sceene withs coners in points p1, p2, p3. with a given pointer to a meterial struct
void add_triangle(object_manager* objm, float p1x, float p1y, float p1z, float p2x, float p2y, float p2z, float p3x, float p3y, float p3z, material* mat, bool invert_normal){
	...
	objm->triangle_objects[objm->triangle_object_count].invert_normal = invert_normal;

	objm->triangle_objects[objm->triangle_object_count].check_intersection = &triangle_intersection;
	objm->triangle_objects[objm->triangle_object_count].get_normal = &triangle_normal;

	objm->sceene_objects[objm->sceene_object_count].type = 't';
	objm->sceene_objects[objm->sceene_object_count].object_pointer = &objm->triangle_objects[objm->triangle_object_count];

	objm->triangle_object_count += 1;
	objm->sceene_object_count += 1;
}

// adds plane to sceene. nx, ny, nz are surface normal values. when setting surface normal it can be set with a length larger
// or smaller than 1. the suface normal is normalized on initalization
void add_plane(object_manager* objm, float px, float py, float pz, float nx, float ny, float nz, material* mat){
	...
	objm->plane_objects[objm->plane_object_count].check_intersection = &plane_intersection;
	...
	objm->sceene_objects[objm->sceene_object_count].object_pointer = &objm->plane_objects[objm->plane_object_count];
...
void add_light(object_manager* objm, float px, float py, float pz, float dr, float dg, float db, float sr, float sg, float sb){
