...
int main(){
	...
	set(&thro_lense_iter_start_pos, camera_object.camera_pos.values[0]+camera_object.distance_to_lense,
	...
	if(camera_object.sub_ray_count > 1){
		sub_ray_iter_w = scale(thro_lense_iter_w, 1.0/((float)camera_object.sub_ray_count));
		sub_ray_iter_h = scale(thro_lense_iter_h, 1.0/((float)camera_object.sub_ray_count));
	}
	...
	vect3d lense_point;
	for(int height_image_index = 0; height_image_index<image_height; height_image_index++){
		for(int width_image_index = 0; width_image_index<image_width; width_image_index++){
			...
			lense_point = add(thro_lense_iter_start_pos, add(scale(thro_lense_iter_w, width_image_index), scale(thro_lense_iter_h, height_image_index)));

			for(int sub_ray_h_index = 0; sub_ray_h_index<camera_object.sub_ray_count; sub_ray_h_index++){
				for(int sub_ray_w_index = 0; sub_ray_w_index<camera_object.sub_ray_count; sub_ray_w_index++){

					// calculate ray direction
					ray_direction = make_unit_vect(camera_object.camera_pos, add(lense_point, add(scale(sub_ray_iter_w, sub_ray_w_index), scale(sub_ray_iter_h, sub_ray_h_index))));
					// find distance to closes object if it exists
					cobj = get_closest_object(&objm, camera_object.camera_pos, ray_direction);
					...
					if(cobj.distance != -1){
						...
						point_color = get_color_of_point(ray_direction, intersection_point, cobj, &objm, &camera_object, ambient_light, 0, 0);
						...
						pixel_color_sum.r = background_color.r * camera_object.sub_ray_count * camera_object.sub_ray_count;
						pixel_color_sum.g = background_color.g * camera_object.sub_ray_count * camera_object.sub_ray_count;
						pixel_color_sum.b = background_color.b * camera_object.sub_ray_count * camera_object.sub_ray_count;
			...
			image_buffer[height_image_index*image_width+width_image_index].r = pixel_color_sum.r / (camera_object.sub_ray_count * camera_object.sub_ray_count);
			image_buffer[height_image_index*image_width+width_image_index].g = pixel_color_sum.g / (camera_object.sub_ray_count * camera_object.sub_ray_count);
			image_buffer[height_image_index*image_width+width_image_index].b = pixel_color_sum.b / (camera_object.sub_ray_count * camera_object.sub_ray_count);
		}
	}
