...
void update_editor(){
	...
	if(ed.edit_mode == MOVE_EDIT_MODE){
		if(ed.event_left_click == true){
			...
			if(ed.moveing_point == -1){
				...
				for(size_t i=0; i<ed.edp_index; i++){
					if(ed.edp[i].point != NULL){
						if(length2d(sub2d(mp, scene_point_to_window_point(*(ed.edp[i].point)))) < 5){
			...
			if(ed.moveing_point != -1){
				*(ed.edp[ed.moveing_point].point) = 
					add3d(window_point_to_scene_point(get_mouse_pos()),
						  sub3d(ed.prev_pos, set_cur_view_vals(get_cur_view_vals(ed.prev_pos)))
					);
			}
		}
		...
		if(ed.object_type == CAMERA_OBJECT){
			...
			tmpl_camera.lense_center = add3d(tmpl_camera.focus, scale3d(add3d(i_hat, j_hat), 50));
			...
			for(size_t i=0; i<ed.edp_index; i++){
				if(ed.edp[i].point != NULL){
					if(length2d(sub2d(mp, scene_point_to_window_point(*(ed.edp[i].point)))) < 5){
						...
						print_vect3d(((sphere*)((ed.edp[i].parent_obj)->object_pointer))->center);
...
void draw_line(vect3d v1, vect3d v2){
	...
	lineRGBA(wd.renderer, (int)p1.v1, (int)p1.v2, (int)p2.v1, (int)p2.v2, 255, 255, 255, 255);
}
...
void draw_camera(camera c){
	...
	vect3d hh = scale3d(direction3d(cross_prod3d(sub3d(c.lense_center, c.focus), i_hat)), c.lense_height/2);
	vect3d hw = scale3d(direction3d(cross_prod3d(sub3d(c.lense_center, c.focus), k_hat)), c.lense_width/2);

	vect3d coner1 = add3d(add3d(c.lense_center, hh), scale3d(hw, -1));
	vect3d coner2 = add3d(add3d(c.lense_center, hh), hw);
	vect3d coner3 = add3d(add3d(c.lense_center, scale3d(hh, -1)), scale3d(hw, -1));
...
void draw_sphere(sphere s){
	...
	circleRGBA(wd.renderer, (int)c.v1, (int)c.v2, (int)s.radius * vw.zoom, 255, 255, 255, 255);
}
...
void draw_axis(){
	...
	if(vw.current_view == TOP_VIEW){
		...
		lineRGBA(wd.renderer, 0, (int)c.v2, WIN_WIDTH, (int)c.v2, 255, 0, 0, 255); // axis
		// arrow
		lineRGBA(wd.renderer, WIN_WIDTH-10, (int)c.v2-5, WIN_WIDTH, (int)c.v2, 255, 0, 0, 255);// arrow
		lineRGBA(wd.renderer, WIN_WIDTH-10, (int)c.v2+5, WIN_WIDTH, (int)c.v2, 255, 0, 0, 255);
		// dist marker tick and value
		lineRGBA(wd.renderer, WIN_WIDTH-47, (int)c.v2-5, WIN_WIDTH-47, (int)c.v2+5, 255, 0, 0, 255);
		d.v1 = WIN_WIDTH-47;
		d.v2 = (int)c.v2;
		tick_pos = window_point_to_scene_point(d);
		sprintf(dist_marker, "%d ", (int)tick_pos.v1);
		stringRGBA(wd.renderer, WIN_WIDTH-45, (int)c.v2-12, dist_marker, 255, 0, 0, 255);
		...
		// arrow
		lineRGBA(wd.renderer, WIN_WIDTH-10, (int)c.v2-5, WIN_WIDTH, (int)c.v2, 0, 255, 0, 255);
		lineRGBA(wd.renderer, WIN_WIDTH-10, (int)c.v2+5, WIN_WIDTH, (int)c.v2, 0, 255, 0, 255);
		// marker
		lineRGBA(wd.renderer, WIN_WIDTH-47, (int)c.v2-5, WIN_WIDTH-47, (int)c.v2+5, 0, 255, 0, 255);
		d.v1 = WIN_WIDTH-47;
		d.v2 = (int)c.v2;
		tick_pos = window_point_to_scene_point(d);
		sprintf(dist_marker, "%d ", (int)tick_pos.v2);
		stringRGBA(wd.renderer, WIN_WIDTH-45, (int)c.v2-12, dist_marker, 0, 255, 0, 255);
	...
	// stringRGBA(wd.renderer, WIN_WIDTH-45, (int)c.v2-12, dist_marker, 255, 255, 255, 255);
	// black pixel in axis crosing
